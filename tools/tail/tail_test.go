...
func runTailWithOutput(path string, cfg Config) (*TailResult, error) {
	return tailFile(path, cfg)
}

func TestTail_Basic(t *testing.T) {
	dir := t.TempDir()
	filePath := createFile(t, dir, "test.txt", "line1\nline2\nline3\nline4\nline5")

	result, err := runTailWithOutput(filePath, Config{XML: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_NLines(t *testing.T) {
	...
	result, err := runTailWithOutput(filePath, Config{LinesFlag: true, Lines: 3})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_LessLinesThanRequested(t *testing.T) {
	dir := t.TempDir()
	filePath := createFile(t, dir, "test.txt", "line1\nline2\nline3")

	result, err := runTailWithOutput(filePath, Config{LinesFlag: true, Lines: 10})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_Bytes(t *testing.T) {
	...
	result, err := runTailWithOutput(filePath, Config{BytesFlag: true, Bytes: 5})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_Directory(t *testing.T) {
	...
	result, err := runTailWithOutput(dir, Config{XML: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_Binary(t *testing.T) {
	dir := t.TempDir()
	...
	result, err := runTailWithOutput(binPath, Config{XML: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTail_MultipleFiles(t *testing.T) {
	...
	_, err := runTail([]string{file1, file2})
	if err != nil {
		t.Fatal(err)
	}
}

func TestTail_XMLValidity(t *testing.T) {
	dir := t.TempDir()
	filePath := createFile(t, dir, "test.txt", "line1\nline2\nline3")
	...
	if result.XMLName.Local != "tail" {
		t.Errorf("expected root element 'tail', got %q", result.XMLName.Local)
	}
}

func TestTail_LanguageEnrichment(t *testing.T) {
	dir := t.TempDir()
