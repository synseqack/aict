...
`aict` is a single Go binary that reimplements 34 Unix CLI tools (`ls`, `grep`, `cat`, `find`, `stat`, `diff`, `sed`, `awk`, `jq`, `tar`, etc.) with structured XML/JSON output designed for AI coding agents.
...
| [Tools Reference](./tools-reference.md) | Documentation for all 34 tools |
